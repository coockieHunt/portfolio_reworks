...
export const useContactForm = () => {
    ...
    const handleReset = () => {
        ...
        if (captchaComponentRef.current)
        ...
        if (data.error) {
            if (data.message === 'Trop de tentatives. Réessayez plus tard.') {
                ...
                trackEvent('Contact Form Error', {
                    reason: data.message || 'API Technical Error',
                });
                addAlert(
                    data.message ||
                        'Le service mail rencontre un problème technique.',
                    '#ffcc00',
                    4000,
                );
            }
    ...
    const CheckData = (data: IMailForm) => {
        if (!data.email || !isValidEmail(data.email)) {
            addAlert('Email invalide.', '#cc3300', 4000);
            return false;
        }
        if (!data.firstName) {
            addAlert('Prénom requis.', '#cc3300', 4000);
            return false;
        }
        if (!data.message) {
            addAlert('Message requis.', '#cc3300', 4000);
            return false;
        }
        return true;
    };
