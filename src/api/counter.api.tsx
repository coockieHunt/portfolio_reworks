...
export async function getThemeRand(): Promise<IApiResponse | null> {
    try {
        const resp = await axios.get<IApiResponse>(`${ApiBaseUrl}/counter/get/THEME_RAND`);
        return resp.data;
    } catch (err) {
...
export async function incrementThemeRand(): Promise<IApiResponse | null> {
    try {
        const resp = await axios.post<IApiResponse>(`${ApiBaseUrl}/counter/increment/THEME_RAND`);
        return resp.data;
    } catch (err) {
        if (axios.isAxiosError(err) && err.response?.status === 429) {
            return {
