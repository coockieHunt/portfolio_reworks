 ...
 */
const ModalItem = ({ modal, index, onClose }: IModalItemProps): React.ReactElement | null => {
...
export const ModalComponent = ({ modals, onClose }: IModalComponentProps): React.ReactElement | null => {
    ...
    useEffect(() => {
        const handleKeyDownGlobal = (event: KeyboardEvent) => {
            const lastOpenedModalIndex = modals.map((m) => m.isOpen).lastIndexOf(true);
            if (lastOpenedModalIndex !== -1) handleKeyDown(event, lastOpenedModalIndex);
        };
    ...
    const modalContent = (
        <AnimatePresence mode="wait">
            {modals.map((modal, index) =>
                modal.isOpen ? (
                    <ModalItem key={`modal-${index}`} modal={{ ...modal, light: modal.light || 'dark' }} index={index} onClose={onClose} />
                ) : null
            )}
        </AnimatePresence>
    );
