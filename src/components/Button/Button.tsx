...
import { ButtonProps, OutlineButtonProps, ScrollToTopProps, IconButtonProps, LinkProps}from './Button.interface';
...
export const OutlineButton: React.FC<OutlineButtonProps> = ({ onClick, color, children, icon, icon_right, disabled }) => {
    return (
        <OutileButtonContainer onClick={!disabled ? onClick : undefined} $colorLine={color} className={disabled ? "disabled" : undefined}>
            {icon_right ? 
 ...
 */
export const ScrollToTop: React.FC<ScrollToTopProps> = ({ hide_top = 400, hide_bottom = 450, auto_hide= true }) => {
    const scrollToTop = () => {
        window.scrollTo({
            top: 0,
            behavior: 'smooth',
