...
fs.readdir(DIR, (err, files) => {
    if (err) {
        return console.error('❌ image directory read error:', err);
    }
            ...
            sharp(filePath)
                ...
                .then((data) => {
                    fs.writeFile(filePath, data, (err) => {
                        if (err)
                            console.error(
                                `❌ Erreur d'écriture pour ${file}:`,
                                err,
                            );

                        const newSize = (data.length / 1024).toFixed(2);
                        const reduction = Math.round(
                            (1 - data.length / originalStats.size) * 100,
                        );

                        console.log(
                            `✅ ${file} : ${originalSize} Ko -> ${newSize} Ko (Gain: ${reduction}%)`,
                        );
                    });
                })
                .catch((err) => {
                    console.error(
                        `❌ image processing error on ${file} :`,
                        err,
                    );
                });
        }
